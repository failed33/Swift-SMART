...
//
	...
	open class PatientListViewController: UITableViewController {
		...
		override open func viewDidLoad() {
			self.tableView.register(
				PatientTableViewCell.self, forCellReuseIdentifier: "PatientCell")
			...
			header.addConstraints(
				NSLayoutConstraint.constraints(
					withVisualFormat: "H:|-[lbl]-|", options: [], metrics: nil,
					views: ["lbl": label]))
			header.addConstraints(
				NSLayoutConstraint.constraints(
					withVisualFormat: "V:|-[lbl]-|", options: [], metrics: nil,
					views: ["lbl": label]))
				...
				if let this = self {
					if let error = error {
						let alertController = UIAlertController(
							title: NSLocalizedString("Loading Patients Failed", comment: ""),
			...
			patientList?.onPatientUpdate = { [weak self] in
				if let this = self {
					...
					this.headerLabel?.text =
						"\(this.patientList!.actualNumberOfPatients) of \(this.patientList!.expectedNumberOfPatients)"
		...
		{
			let cell =
				tableView.dequeueReusableCell(withIdentifier: "PatientCell", for: indexPath)
				as! PatientTableViewCell
			if let section = patientList?[indexPath.section] {
				cell.represent(section[indexPath.row])

				let marker = min(
					patientList!.expectedNumberOfPatients, UInt(section.offset + indexPath.row + 10)
				)
		...
		override open func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int)
			-> String?
		{
			if let section = patientList?[section] {
	...
	extension PatientList {
		...
		subscript(indexPath: IndexPath) -> ModelsR5.Patient? {
	...
	class PatientTableViewCell: UITableViewCell {
		...
		func represent(_ patient: ModelsR5.Patient?) {
			textLabel?.text = patient?.displayNameFamilyGiven

			// birthday and age
			if let bdate = patient?.birthDate {
				let attr = NSMutableAttributedString(
					string: "\(bdate.value?.description ?? "")  (\(patient!.currentAge))",
			...
			if let pat = patient {
				let gender =
					accessoryView as? UILabel
					?? UILabel(frame: CGRect(x: 0, y: 0, width: 38, height: 38))
